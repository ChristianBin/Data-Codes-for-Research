# -*- coding: utf-8 -*-
"""
Created on Thu Jul  7 09:42:48 2022
...
def set_variables(duc_model, instance_index):
    ...
    #setting start up cost decision variable for each generator g in [1, G] for each period t in [2, T]
    su_var = {(g,t): duc_model.continuous_var(name = 'su_{0}_{1}'.format(g,t), lb = 0) \
              for g in range(1, uc_total_gen_num[instance_index-1] + 1) for t in range(2, uc_opt_period + 1)}
    
    #setting shut down cost decision variable for each generator g in [1, G] for each period t in [2, T]  
    sd_var = {(g,t): duc_model.continuous_var(name = 'sd_{0}_{1}'.format(g,t), lb = 0) \
              for g in range(1, uc_total_gen_num[instance_index-1] + 1) for t in range(2, uc_opt_period + 1)}
    ...
    return x_var, y_var, su_var, sd_var, fx_var
    
def add_polytope_constraints(duc_model, x_var, y_var, su_var, sd_var, fx_var, instance_index):
    ...
    for gen_type in range(0, 8):
        ...
        if uc_gen_num[instance_index-1, gen_type] > 0:
            ...
            #constriant (1a) and (1b): minimum on and off period constraints for each generator g in [1, G]
            for t in range(2, uc_opt_period + 1):
                #add (1a) constraint
                s1 = min(uc_opt_period, t + uc_min_on[gen_type] - 1)
                duc_model.add_constraints((-y_var[g,t-1] + y_var[g,t] - y_var[g,k] <= 0 \
                            for k in range(t, s1+1) for g in range(start_index + 1, end_index + 1)), \
                            names = 'cstr_min_on')
                #add (1b) constraint
                s2 = min(uc_opt_period, t + uc_min_off[gen_type] - 1)
                duc_model.add_constraints((y_var[g,t-1] - y_var[g,t] + y_var[g,k] <= 1 \
                                           for k in range(t, s2+1) for g in range(start_index + 1, \
                                           end_index + 1)), names = 'cstr_min_off')
            
            #constraint (1c): generation lower bound
            ...
            #constraint (1d): generation upper bound
            duc_model.add_constraints((x_var[g,t] - uc_gen_ub[gen_type]*y_var[g,t] <= 0 \
                                       for t in range(1, uc_opt_period +1) for g in range(start_index + 1, \
                                       end_index + 1)), names = 'cstr_gen_ub')
            
            #constraint(1e): ramp up limitation for successive period for each type of generator
            ...
            #constraint (1f): ramp down limitation for successive period for each type of generator
            ...
            #constraint (1g): start up cost constraint for su_var[g][t] in each period [2, T] \
            #for each generator [1, G], we set it as a decision variable su[g][t] instead of adding 
            #a binary variable u[g][t] to denote startup or not in period t for generator g
            duc_model.add_constraints((su_var[g,t] >= uc_unit_startup_cost[gen_type]*(y_var[g,t] - \
                                      y_var[g,t-1]) for t in range(2, uc_opt_period + 1) for g in \
                                      range(start_index + 1, end_index + 1)), names = 'cstr_su_cost')
                
            #constraint(1h): shut down cost constraint for sd_var[g][t] in each period [2, T] for each generator [1, G] , 
            #we set it as a decision variable sd[g][t] instead of adding a binary variable
            #z[g][t] to denote shutdown or not in period t for generator g
            duc_model.add_constraints((sd_var[g,t] >= uc_unit_shutdown_cost[gen_type]*(y_var[g,t-1] - \
                                      y_var[g,t]) for t in range(2, uc_opt_period + 1) for g in \
                                      range(start_index + 1, end_index + 1)), names = 'cstr_sd_cost')
            
            #constraint (1i): generation cost constraint fx[g][t] in each period [1, T] for each generator
            ...
            #add two-period convex hull cuts
    ...
    #constriant (1j): total generation amount for all generators at each period should be equal to
    ...
    #constraint (1k): spinning reserve constraint
...
def set_obj_function(duc_model, x_var, y_var, su_var, sd_var, fx_var, instance_index):
    '''setting the objective function for the model'''
    
    #summing up the total start up and shut down cost
    total_start_shut_cost = duc_model.sum(su_var[g,t] + sd_var[g,t] for g in range(1, \
                                uc_total_gen_num[instance_index-1] + 1) for t in range(2, uc_opt_period+1))
...
def print_sol_info(duc_model_sol):
    '''Print out the solution and solving information'''
    
    duc_model_sol.display()
    #duc_model.print_solution()
...
def build_model(usercutpool, usercutcallback, instance_index):
    ...
    x_var, y_var, su_var, sd_var, fx_var = set_variables(m, instance_index)
    #adding polytope constraints
    add_polytope_constraints(m, x_var, y_var, su_var, sd_var, fx_var, instance_index)
    ...
    set_obj_function(m, x_var, y_var, su_var, sd_var, fx_var, instance_index)
    ...
    if usercutpool:
        ...
        add_cutpool_cuts(m, x_var, y_var, instance_index)
